...
/** Spring 应用启动器，提供了大量的静态方法，可以很容易运行一个独立的 Spring 应用程序 <br>
 * <p> Class that can be used to bootstrap and launch a Spring application from a Java main
...
public class SpringApplication {
	...
	/**
	 * 主要的 java config 类的数组，{@link org.springframework.context.annotation.Configuration}
	 */
	...
	/**
	 * 获得是调用了哪个 #main(String[] args)，用来打印日志，说明是通过这个类来启动 Spring 应用的
	 * <p> class org.springframework.boot.MVCApplication
	 */
	...
	/**
	 * web 应用类型，调用 {@link WebApplicationType#deduceFromClasspath()} 方法，
	 * 通过 classpath，判断 web 应用类型。
	 * <p> {@link #createApplicationContext()} 将根据该值（类型），创建不同类型的
	 * {@link ConfigurableApplicationContext} 对象
	 */
	...
	/**
	 * {@link ApplicationContextInitializer} 数组
	 *
	 * {@literal 'META-INF/spring.factories'} 下的 org.springframework.context.ApplicationContextInitializer
	 *
	 * <p> 在 Spring MVC 下，有如下默认初始化对象：
	...
	/**
	 * {@link ApplicationListener} 数组
	 *
	 * {@literal 'META-INF/spring.factories'} 下的 org.springframework.context.ApplicationListener
	 *
	 * <p> 在 Spring MVC 下，有如下默认初始化对象：
	...
	public SpringApplication(ResourceLoader resourceLoader, Class<?>... primarySources) {
		...
		/**
		 * 推断容器类型 {@link WebApplicationType}
		 * {@link WebApplicationType#SERVLET}:内嵌的 Servlet Web 应用。例如说，Spring MVC 。
		 * {@link WebApplicationType#NONE}:非内嵌的 Web 应用
		 * {@link WebApplicationType#REACTIVE}:内嵌的 Reactive Web 应用。例如说，Spring Webflux 。
		 */
		this.webApplicationType = WebApplicationType.deduceFromClasspath();
		/**
		 * 初始化 initializers 属性。通过 SPI，获取 META-INF/spring.factories 下配置的
		 * {@link ApplicationContextInitializer}
		 */
		setInitializers((Collection) getSpringFactoriesInstances(ApplicationContextInitializer.class));
		/**
		 * 初始化 listeners 属性。通过 SPI，获取 META-INF/spring.factories 下配置的
	...
	public ConfigurableApplicationContext run(String... args) {
		...
		/**
		 * 获得 {@link SpringApplicationRunListener} 的数组，并启动监听。
		 * META-INF/spring.factories 下的 org.springframework.boot.SpringApplicationRunListener
		...
		try {
			...
			// 异常报告器 org.springframework.boot.SpringBootExceptionReporter
			exceptionReporters = getSpringFactoriesInstances(SpringBootExceptionReporter.class,
					new Class[] { ConfigurableApplicationContext.class }, context);
			/**
			 * 主要是调用所有初始化类的 initialize 方法，其中完成了 bean 注册
			 */
			...
			/**
			 * 通知 {@link SpringApplicationRunListener} 的数组，Spring 容器启动完成。
			 */
			listeners.started(context);
			/**
			 * 调用 {@link ApplicationRunner} 或者 {@link CommandLineRunner} 的运行方法。
			 */
	...
	private <T> Collection<T> getSpringFactoriesInstances(Class<T> type, Class<?>[] parameterTypes, Object... args) {
		...
		/**
		 * 加载指定类型对应的，在 {@literal 'META-INF/spring.factories'} 指定的全限定的类名
		 */
	 ...
	 * <p> Add, remove or re-order any {@link PropertySource}s in this application's
	 * environment.
	 * @param environment this application's environment
	 * @param args arguments passed to the {@code run} method
	...
	/**
	 * spring boot 激活 profile 的几种方式：https://my.oschina.net/u/1469495/blog/1522784
	 * <p> Configure which profiles are active (or active by default) for this application
	...
	/**创建 Spring 容器
	 * 根据 {@link WebApplicationType} 类型，获得对应的 {@link ApplicationContext} 对象
	 * <p> Strategy method used to create the {@link ApplicationContext}. By default this
	...
	/**初始化 ApplicationContextInitializer
	 * <p> Apply any {@link ApplicationContextInitializer}s to the context before it is
	 * refreshed.
	...
	/**加载 BeanDefinition 们
	 * <p> Load beans into the application context.
	 ...
	 * <p> Get the bean definition registry.
	 * @param context the application context
	...
	/**创建 {@link org.springframework.boot.BeanDefinitionLoader} 对象
	 * <p> Factory method used to create the {@link BeanDefinitionLoader}.
